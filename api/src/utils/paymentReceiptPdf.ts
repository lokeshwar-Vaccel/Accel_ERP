...
import { IAMCInvoicePayment } from '../models/AMCInvoicePayment';
...
// Interface for AMC Invoice Payment
interface PopulatedAMCInvoicePayment extends Omit<IAMCInvoicePayment, 'amcInvoiceId' | 'customerId' | 'createdBy'> {
  amcInvoiceId: {
    _id: string;
    invoiceNumber: string;
    grandTotal: number;
...
// Generate PDF for AMC invoice payments
export const generateAMCInvoicePaymentReceiptPDF = (payment: PopulatedAMCInvoicePayment): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ 
        size: 'A4', 
        margin: 40,
        info: {
          Title: `AMC Invoice Payment Receipt - ${payment.invoiceNumber}`,
          Author: 'Sun Power Services',
          Subject: 'Payment Receipt',
          Creator: 'Sun Power Services ERP'
        }
      });
      ...
      doc.fontSize(14)
         .font('Helvetica')
         .fill('#6B7280')
         .text('AMC Invoice Payment', { align: 'center' })
         .moveDown(1);
      ...
      const rightColumn = margin + 250;

      const receiptInfoY = doc.y;
      doc.rect(margin, receiptInfoY, contentWidth, 80)
         .fill('#F9FAFB')
         .stroke('#E5E7EB');

      doc.fontSize(12).font('Helvetica-Bold').fill('#374151');
      doc.text('Receipt Information', margin + 15, receiptInfoY + 8);

      doc.fontSize(10).font('Helvetica').fill('#374151');
      doc.text(`Receipt Number: ${payment.receiptNumber || payment._id}`, leftColumn, receiptInfoY + 25);
      doc.text(`Payment Date: ${payment.paymentDate.toLocaleDateString('en-IN')}`, leftColumn, receiptInfoY + 45);
      doc.text(`Payment Time: ${payment.paymentDate.toLocaleTimeString('en-IN')}`, leftColumn, receiptInfoY + 65);

      doc.text(`Amount: ₹${payment.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, rightColumn, receiptInfoY + 25);
      doc.text(`Currency: ${payment.currency}`, rightColumn, receiptInfoY + 45);
      doc.text(`Status: ${payment.paymentStatus.toUpperCase()}`, rightColumn, receiptInfoY + 65);
      ...
      let customerInfoY = customerY + 40;
      doc.fontSize(10).font('Helvetica').fill('#374151');
      doc.text(`Name: ${payment.customerId.firstName} ${payment.customerId.lastName}`, leftColumn, customerInfoY);
      customerInfoY += 20;
      doc.text(`Email: ${payment.customerId.email}`, leftColumn, customerInfoY);
      customerInfoY += 20;
      if (payment.customerId.phone) {
        doc.text(`Phone: ${payment.customerId.phone}`, leftColumn, customerInfoY);
        customerInfoY += 20;
      }
      if (payment.customerId.address) {
        doc.text(`Address: ${payment.customerId.address}`, leftColumn, customerInfoY);
        customerInfoY += 20;
      }
      ...
      doc.fontSize(12)
         .font('Helvetica-Bold')
         .text('AMC Invoice Summary', { underline: true })
         .moveDown(0.5);

      const invoiceSummary = [
        ['AMC Invoice Total:', `₹${payment.amcInvoiceId.grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`],
        ['Amount Paid:', `₹${payment.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`],
        ['Remaining Amount:', `₹${(payment.amcInvoiceId.grandTotal - payment.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`]
      ];

      invoiceSummary.forEach(([label, value]) => {
        doc.fontSize(10)
           .font('Helvetica-Bold')
           .text(label, 50, doc.y)
           .font('Helvetica')
