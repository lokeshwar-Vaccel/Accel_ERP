...
// Schema for DG Proforma Item
export const dgProformaItemSchema = Joi.object({
  ...
  hsnNumber: Joi.string().allow('').trim().default('')
});
...
// Schema for billing/shipping address
...
const transportChargesSchema = Joi.object({
  ...
  hsnNumber: Joi.string().default('998399').trim(),
  gstRate: Joi.number().min(0).max(100).default(18).messages({
...
// Schema for creating DG Proforma
export const createDGProformaSchema = Joi.object({
  ...
  dgQuotationNumber: Joi.string().allow('').optional().messages({
  ...
  proformaDate: Joi.date().required().messages({
    'date.base': 'Proforma date must be a valid date',
    'any.required': 'Proforma date is required'
  }),
  ...
  items: Joi.array().items(dgProformaItemSchema).min(1).required().messages({
...
// Schema for updating DG Proforma
export const updateDGProformaSchema = Joi.object({
  customer: Joi.string(),
  customerEmail: Joi.string().email().allow('').optional().trim(),
  customerAddress: addressSchema,
  billingAddress: billingShippingAddressSchema,
  ...
  poNumber: Joi.string().allow('').trim(),
  poFromCustomer: Joi.string().allow(null),
  proformaDate: Joi.date().messages({
    'date.base': 'Proforma date must be a valid date'
  }),
  ...
  dispatchedThrough: Joi.string().allow('').trim(),
  termsOfDelivery: Joi.string().allow('').trim(),
  items: Joi.array().items(dgProformaItemSchema).min(1).messages({
    'array.min': 'At least one item is required'
  }),
  additionalCharges: additionalChargesSchema,
  transportCharges: transportChargesSchema,
...
export const getDGProformasQuerySchema = Joi.object({
...
export const exportDGProformasQuerySchema = Joi.object({
  search: Joi.string().allow('').trim(),
  startDate: Joi.date().iso(),
  endDate: Joi.date().iso(),
  status: Joi.string().valid('Draft', 'Sent', 'Accepted', 'Rejected', 'Expired'),
